...
class DCNv4_DyHead(nn.Module):
    def __init__(
            ...
            **kwargs):
        """
        DCNv4 Module with PyTorch implementation
        """
        super().__init__()
        if channels % group != 0:
            raise ValueError(
        ...
        # Use regular deformable convolution as fallback
        # Note: This is a simplified version and won't have the exact same behavior as DCNv4
        ...
        if not without_pointwise:
            self.output_proj = nn.Conv2d(channels, channels, kernel_size=1, bias=output_bias)
        
        self._reset_parameters()

    def _reset_parameters(self):
        nn.init.xavier_uniform_(self.deform_conv.weight.data)
        if self.deform_conv.bias is not None:
            nn.init.constant_(self.deform_conv.bias.data, 0.)
    ...
    def forward(self, x, offset_mask):
        """
        Forward pass that works without custom CUDA extensions
        """
        N, C, H, W = x.size()
        
        # Extract offset information from offset_mask to guide the convolution
        offset_scale_factor = self.offset_scale
        
        # Đảm bảo offset_mean có cùng kích thước với x bằng cách resize
        offset_mean = offset_mask.mean(dim=1, keepdim=True) * offset_scale_factor
        
        # Resize offset_mean để phù hợp với kích thước của x
        if offset_mean.shape[2:] != x.shape[2:]:
            offset_mean = F.interpolate(offset_mean, size=(H, W), mode='bilinear', align_corners=False)
        
        # Nhân với x
        x = x * (1.0 + torch.tanh(offset_mean))
        
        # Apply standard convolution as fallback
        x = self.deform_conv(x)
        
        # Apply pointwise projection if needed
        if not self.without_pointwise:
            x = self.output_proj(x)
