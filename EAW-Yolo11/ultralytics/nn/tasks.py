...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Detect_DyHead, Detect_LSCD)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s).to(torch.device('cuda')))])   # forward
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m in {C3k2, EC3k2, FC3k2, EFC3k2, C3k2_DCNv2, C3k2_Faster, C3k2_Faster_EMA, C3k2_Faster_CAA, C3k2_Faster_CGLU, C3k2_Star, C3k2_Star_EMA, C3k2_Star_CAA, C3k2_CaFormerCGLU}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
        elif m is AIFI:
            ...
            if m in {Detect, Detect_DyHead, Detect_LSCD, Segment, Pose, OBB}:
                m.legacy = legacy
